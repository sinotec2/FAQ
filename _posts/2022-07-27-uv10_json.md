...
### 目的
- 這個版本直接由grib2檔案讀取地面風進行內插
  - 使用gribby模組來開啟、讀取
  - 固定檔名：M-A0064-006.grb2。每6小時下載，約與目前時間一致。
...
```python
$ diff grbuv10_json.py uv10_json.py
1c1
...
>   var=nc.variables[uv[ir]][0,:,:]
76,77c60,61
< if '61' in fname:fnameO='current-wind-surface-level-cwb-15K.json'
< if '64' in fname:fnameO='current-wind-surface-level-cwb-3K.json'
---
> if 'd01' in fname:fnameO='current-wind-surface-level-cwb-15K.json'
> if 'd03' in fname:fnameO='current-wind-surface-level-cwb-3K.json'
```
...
#$ cat earth_cwbwrf.cs 
i=06
cd /Users/Data/javascripts/D3js/earth/public/data/weather/current
for dom in 4;do
  rm  -f M-A006${dom}-0$i.grb2
  /opt/local/bin/wget https://cwbopendata.s3.ap-northeast-1.amazonaws.com/MIC/M-A006${dom}-0$i.grb2
  ./grbuv10_json.py
done
```
### crontab內容
...
# earth CWB_WRF
0 3,9,15,21 * * * /Users/Data/javascripts/D3js/earth/public/data/weather/current/earth_cwbwrf.cs
```
