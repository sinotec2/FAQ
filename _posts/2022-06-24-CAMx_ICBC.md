...
{% include warning.html content="CAMx時間沒有硬性規定，而是在CAMx.in內設定。" %}
...
- 格式範例詳見bndary.f，其目的在將全月逐時的邊界濃度檔案，按照日期進行切割以進行逐日的CAMx模擬測試。
...
{% include download.html content="Fortran程式：[bndary.f](https://github.com/sinotec2/Focus-on-Air-Quality/blob/main/CAMx/ICBC/bndary.f)" %}
