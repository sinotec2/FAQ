...
for i in 1 2 3;do
  if [ -e allEA_$i.grib ];then rm allEA_$i.grib;fi
  ./get_All.py $dt $hr $i >& /dev/null
  $ncl allEA_$i.grib
  $ncks -O -d forecast_time0,4 allEA_$i.nc allEA_$i.nc_0
done
./merge.cs
./grb2icon.py
./grb2bcon.py
...
# 執行gfs檔案下載
cd $gfs
for ((i=0;i <= 240; d+=3));do
  iii=$(printf "%03d" $i)
  file=gfs.t00z.pgrb2.1p00.f$iii
  if [ -e $file ];then rm $file;fi
  $wget -q $root$dir$file
...
BEGD=$(date +%Y-%m-%d)
ENDD=$(date -d "$today +10days" +%Y-%m-%d)
...
## 起迄年、月、日
yea1=$(date +%Y);mon1=$(date +%m);day1=$(date +%d);
yea2=$(date -d "$today +10days" +%Y);mon2=$(date -d "$today +10days" +%m);day2=$(date -d "$today +10days" +%d);
for i in 0 1 2;do
  cd $gfs/${DOM[$i]}
  ## 置換模版中的起迄日期
  ...
  if [ $i -eq 0 ] || [ $i -eq 1 ];then
    ...
    ## 執行CWBWRF結果的內插
    EXE=mk_metoaT.py
    for j in {0..79};do sub python $EXE $i;sleep 10s;done
    ## 等候所有的程序都完成，才執行下一動作。
    while true;do
      n=$(psg ${EXE}|wc -l)
      if [ $n -lt 2 ];then
  ...
  # wrf
  LD_LIBRARY_PATH=/opt/netcdf/netcdf4_gcc/lib /opt/mpich/mpich3_gcc/bin/mpirun -np 96 /opt/WRF4/WRFv4.2/main/wrf.exe >& /dev/null

  # mcip
  cd $cmaq/data/wrfout
  j=0;for f in $(ls $gfs/${DOM[$i]}/wrfout*);do ln -sf $f wrfout_d0${$d}_$j;j=$(( $j + 1 ));done
  cd $cmaq
  csh run_mcipMM_RR_DM.csh 08 8 d0${d} >&/dev/null &
done
```
