...
sidebar:
  nav: layouts
date:  2022-10-20
modify_date: 2022-10-21 14:21:57
---
...
- NCO程式：其中的ncks、ncrcat及ncatted，進行模擬結果檔案的切割、連接、以及STIME全域變數的修改。參考[使用手冊](https://nco.sourceforge.net/nco.pdf)、或[教學範例](http://research.jisao.washington.edu/data_sets/nco/)
...
- 月曆日(calendar day)轉換成儒略日(julian day)：(`date -d "$?d" +%Y%j`)
  - 避免使用[cal2jul.f](https://sinotec2.github.io/FAQ/2022/10/21/cal2jul.html)，降低程式的複雜度、跨機器需重新編譯的困擾。
...
```bash
  jbh=0; test $bd == ${dates[$jd1]} && jbh=$(( $bh - 1))
  jeh=23; test $ed == ${dates[$jd1]} && jeh=$(( $eh - 1 ))
```
...
a='$NCRCAT -O '$(echo ${fnames[@]})' $s.nc'
eval $a
NCATTED -a STIME,global,o,f,$(( $bh - 1))0000. $s.nc
rm ${s}_*.png;for i in $( seq 0 $len);do rm ${fnames[$i]};done
if [[ $d == 1 || $d == 2 ]];then ~/bin/join_nc.py $s.nc;fi
