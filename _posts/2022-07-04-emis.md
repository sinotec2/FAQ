...
sidebar:
  nav: layouts
modify_date: 2022-07-08 13:29:56  
---
...
## 點源轉檔結果之確認
1. uamiv及IOAPI ncf均可以用verdi進行展示，包括空間分布、時間序列、以及局部數字之探索(詳[VERDI使用說明](https://sinotec2.github.io/Focus-on-Air-Quality/utilities/Graphics/VERDI/VERDI_Guide/) )
2. ptsource可以用pncdump或者執行pt2emNest轉成uamiv格式用verdi檢視
3. pt2em也有python版本

### pt2em.f
CAMx point_source format

1. camx600以後的版本將其輸入檔都轉成uamiv格式，但是點源檔案仍然有許多參數是在表頭不能適用，因此在圖形展示上造成不小困擾。
