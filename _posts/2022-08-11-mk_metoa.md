...
sidebar:
  nav: layouts
date:  2022-08-11
modify_date: 2022-08-19 10:49:29
mermaid: true
---
## 背景
...
:-:|:-:|:-:|:-:|-
d01|CWBWRF_15Km範圍|取最近值|45Km|中亞到日本、印尼到蒙古範圍，與CAMS預報解析度接近
d02|CWBWRF_3Km範圍|取最近值|9Km|南中國 ～ 台灣之中間層
d03|CWBWRF_3Km範圍|線性內插值|3Km|環保署公版模式範圍

- 策略上，d01主要是引用CAMS的預報結果，d02是中間過渡，d03才是真正需要仔細預報空品的範圍。因此，CWBWRF結果的引用，前二者使用較粗略但快速的方式，直接取最近點的模擬結果進行FDDA，第三層則使用內插值，一方面有較高的精度，一方面網格數較少，計算時間還可以容忍(還是必須平行計算)。

### WPS 之網格設定

### mk_nearst_csv.py
- 參考[CAMS預報數據寫成CMAQ邊界檔 ](https://sinotec2.github.io/FAQ/2022/08/16/CAMS_bc.html)的作法，找到CMAQ網格點最近的CWBWRF點，並在時間迴圈之外將其紀錄下來，不必每次計算。
