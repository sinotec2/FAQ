...
- Markdown and Visual Studio Code, [code.visualstudio.com](https://code.visualstudio.com/docs/languages/markdown)

## [VS Code][vsc_wiki]安裝

### 免安裝版本
- Code也可以直接在browser上執行。功能與安裝版一樣。
- 上網編輯雲端檔案，可以說是現代數位筆記系統的重要功能之一(google doc、evernote etc.)。但網路版Code與桌機版一樣，可以選擇編輯本地版本或在GitHub界面直接編輯，在本地複製一份進行編輯還是比較安全些。
...
### 從GitHub Desktop啟動VS Code
- 作為GitHub預設的編輯軟體，在GitHub官網上、或是從GitHub Desktop歡迎畫面上，都可以啟動指定[Repo][repo]為根目錄的Code作業，如圖5所示。
...
|:--:|
| <b>圖5 在GitHub Desktop歡迎畫面開啟指定[Repo][repo]為根目錄的Code作業</b>| 

- 當然在本地複製雲端存放庫的方法不是只有這一種。
  1. 可以直接以命令列指令複製(`git clone https:...git`)
  1. 使用GitHub Desktop 程式來[開啟](https://docs.github.com/cn/desktop/contributing-and-collaborating-using-github-desktop/adding-and-cloning-repositories/cloning-and-forking-repositories-from-github-desktop)
...
- 左上(圖6)
  1. 檔案總管(Explorer)。又分為[Repo][repo](以下所有的目錄文件)、[大綱](#大綱開合及移動)(選取md檔案的大綱)、時間表(作業過程的儲存版本)。如有未存檔案會出現藍色圓點數字，提示要存檔再離開。
...
|:--:|:--:|
| <b>圖6 [VS Code][vsc_wiki]界面左上角功能</b>| <b>圖7 左下角功能</b>|
...
|:--:|:--:|
| <b>圖8 [VS Code][vsc_wiki]界面右上角功能、文件右上側為整個文件之[縮圖預覽](#縮圖顯示)(含[大綱開合](#大綱開合及移動)及[編輯](#編輯窗格)、[尋找](#文件內搜尋取代)之效果)</b>| <b>圖9 右下角功能</b>|
...
- 文件版本比較之顏色表示(行號右側之垂直線條、[縮圖](#縮圖顯示)左側、拉桿左側，如圖10)
  - 相對於GitHub上的版本
  - 綠色：此次新增 
  - 藍色：修改
  - Push更新GitHub之後，這些顏色就會消失

| ![find_snip.PNG](https://github.com/sinotec2/Focus-on-Air-Quality/raw/main/assets/images/find_snip.png "圖10 右側文件縮圖與拉桿上顏色的分布：尋到之關鍵字詞(橘色底橫線與方塊)、新增文字區塊(綠色)、修改(藍色)")|
|:--:|
| <b>圖10 右側文件縮圖與拉桿上顏色的分布：[搜尋](#文件內搜尋取代)到之關鍵字詞(橘色底橫線與方塊)、新增文字區塊(綠色)、修改(藍色)</b>| 
...
## 預覽
- Code有2個預覽畫面
  - 一者在文件編輯器的右側拉桿邊，類似MS Word的縮圖顯示，如[圖10](#編輯窗格)所示。
