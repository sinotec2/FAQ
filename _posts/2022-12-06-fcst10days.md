...
sidebar:
  nav: layouts
date:  2022-12-06
modify_date: 2022-12-08 21:04:45
---
...
- 增加跨網路節點似乎沒辦法減少太多執行時間。可能是這2個工作站的速度差異還頗大的，造成速度瓶頸。
- 此方案以dev2執行10天中的前半段、後半段與結果整合，則由速度較快的devp負責執行。
- dev2的啟動機制：當上一層最後一天的PMs結果都處理好了，就開始執行下一層的`CCMS`與`run_bcon`，重複執行第2、3層的`CCTM`。
- devp只要dev2一開始執行`CCTM`，判斷邏輯以最新的CCTM_ACONC檔名時間標籤，正好等於第一天。
- 下一層各日BCON的切割納入平行運作，以其所有檔案大小的總和為判斷依據。如果總和正確，則進行`ncrcat`整併、準備下一層`CCTM`的執行。
