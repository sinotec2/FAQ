...
- 背景執行metgrid需要while指令來確認所有執行檔真的結束了，才能啟動real和wrf。
- 這個工作由wait_exe來執行(詳[以下說明](#確認執行完成之小工具wait_exe))
...
- 重新整理運作邏輯，趁著切割下一層BC與IC時，讓combine單獨在背景運作。
- 第3層因為不必執行run_bcon/run_icon，需確認combine確實完成了，再執行下一個動作(cmaq_json)，使用[wait_exe](#確認執行完成之小工具wait_exe)。
...
- 所以只要將複製的動作分散在各日的python檔內執行即可。這就是cmaq_jsonByDay.py的特點。
...
grid09|20 10|200|<p>devp:100</p><p>dev2:100</p>|2:00
