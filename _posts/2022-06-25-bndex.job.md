...
- [pick](https://github.com/sinotec2/Focus-on-Air-Quality/blob/main/CAMx/PostProcess/pick_avrg.f)讀取初始日期。現在這支程式的功能完全可以被[pncdump][pseudonetcdf]取代。
  - `-H`：header only
  - `-f lateral_boundary`：指定檔案格式，詳見[pncgen/pncdump 所有可接受的格式-CAMx](https://sinotec2.github.io/Focus-on-Air-Quality/utilities/netCDF/pncgen/#camx)
...
## [BNDEXTR.py]()
- [PseudoNetCDF][pseudonetcdf]模組會將lateral_boundary格式檔案的污染物濃度變數名稱，加上東西南北方向標籤。因此變數變成原來的4倍。
  - SO2 -> SOUTH_SO2
  - NOSPEC=60 -> NVARS = 240
- 按實際大小給定矩陣的格數 
...
- 執行流程
  1. pncgen將lateral_boundary格式檔案轉成nc檔案模版備用
  1. 執行python讀取上層網格濃度檔、進行切割、截取及時間、空間的內插(參考[EAC4檔案轉成4階邊界檔案](https://sinotec2.github.io/Focus-on-Air-Quality/AQana/GAQuality/ECMWF/grb2bc))
  1. 按照模版輸出成nc檔案
  1. 使用pncgen將nc檔轉成lateral_boundary格式檔案
