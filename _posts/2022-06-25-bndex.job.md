...
- [pick](https://github.com/sinotec2/Focus-on-Air-Quality/blob/main/CAMx/PostProcess/pick_avrg.f)讀取初始日期。現在這支程式的功能完全可以被[pncdump][pseudonetcdf]取代。
  - `-H`：header only
  - `-f lateral_boundary`：指定檔案格式，詳見[pncgen/pncdump 所有可接受的格式-CAMx](https://sinotec2.github.io/Focus-on-Air-Quality/utilities/netCDF/pncgen/#camx)及[lateral_boundary格式][bnd]
...
{% include warning.html content="PseudoNetCDF的[Write.py][Write]需要修改，新版netCDF不允許設定全域屬性*NAME*" %}
