...
- 由於未來將使用mozart2camx程式做座標系統、網格系統及檔案格式轉檔(nc/m3ioapi→uamiv)，因此有必要先將文字檔轉成nc檔，將逐月橫向的文字檔，轉成nc檔案的時間軸
...
- area：
  - 每個等經緯度網格範圍的面積(約略值,KM^2)(line 88~102,)
...
程式又稱為NCF2IOAPI。此舉將mozart等一般的nc檔案(前述的模版frame.nc)，轉成USEPA的標準格式(model3/IOAPI檔案，可以用verdi開啟)。其後再進行切割或類別間的合併。

NCF2IOAPI不會改變檔案的格點數、網格系統，主要改變的是一般性的變數的名稱，如原點是nc.XORIG、間距是nc.XCELL、時間標籤是TFLAG等，IOAPI有自己成套的convention。
...
- multavrg ：
	...
	- 結果為gmole/月
- add_time.f加入時間標籤(僅24小時，可供非日期鑑別之CAMx模擬)
- 重複執行add.cs即可完成全年轉檔
	- 注意：
...
3. DATE :雖然是每格1個月有1個數字，但是m3.nc是嚴格的等間距檔案，此處設為30日(720小時)，反應在ND
	1. macOS/centos的date略有差異
...
```
kuang@master /nas1/TEDS/REAS3.1/join_spec 
$ cat -n add_ttime.f 
     1        INCLUDE  '/home/kuang/bin/PARAMS.CMD' 
     2        INCLUDE  '/home/kuang/bin/CHPARM.CMD' 
     3        INCLUDE  '/home/kuang/bin/CNTROL.CMD' 
     4        INCLUDE  '/home/kuang/bin/FILCON.CMD' 
     5        INCLUDE  '/home/kuang/bin/SEGTAB.CMD' 
     6        INCLUDE  '/home/kuang/bin/NETDEP.CMD' 
     7        INCLUDE  '/home/kuang/bin/BALANC.CMD' 
     8        INCLUDE  '/home/kuang/bin/LOCPTR.CMD' 
     9        INCLUDE  '/home/kuang/bin/MSCAL.CMD' 
    10        character*4 NAMAV,MSPEC,MRUNID 
    11       integer,parameter::fmax=300 
    12       integer Itmp(4) 
    13       CHARACTER*100 NAM0(fmax) ! input/output file names 
    14       PARAMETER (NI=600,NJ=600) 
    15       dimension MO3(10) 
    16       character*4,allocatable:: SPNAME(:,:) 
    17       integer,allocatable::ndate2(:),ndlast2(:),simtim(:) 
    18       real,allocatable::ttime2(:),ttlast2(:) 
    19       real,allocatable::A1(:,:) 
    20 
    21       NUAV=41 
    22       narg=iARGc () 
    23       if(narg.ne.2) stop 'input the filename, mon' 
    24       do i=1,2 
    25         call getarg(i,nam0(i)) 
    26       enddo 
    27         open(11,file=trim(nam0(1)), 
    28      +      form='unformatted',convert='BIG_ENDIAN',STATUS='unknown') 
    29       read(nam0(2),*)mon 
    30         open(12,file=trim(nam0(1))//'_'//trim(nam0(2)), 
    31      +      form='unformatted',convert='BIG_ENDIAN',STATUS='unknown') 
    32       nfile=1 
    33       allocate(ndate2(nfile)) 
    34       allocate(ndlast2(nfile)) 
    35       allocate(ttime2(nfile)) 
    36       allocate(ttlast2(nfile)) 
    37       allocate(simtim(2*nfile)) 
    38       ird=1 
    39         READ (IRD+10) NAMAV, MRUNID, NOSEG, NOSPEC, 
    40      +    NDATE2(ird), TTIME2(ird), 
    41      $    NDLAST2(ird), TTLAST2(ird) 
    42          print*,NDATE2(ird), TTIME2(ird), NDLAST2(ird), TTLAST2(ird) 
    43         allocate(SPNAME(10,NOSPEC)) 
    44         READ (IRD+10) XUTM, YUTM, NZONE, XORG, YORG, DELTAX, DELTAY, 
    45      $    NOXG, NOYG, NOZ, NVLOW, NVUP, DZSURF, DZMINL, DZMINU 
    46         NOSEG=1 
    47         READ (IRD+10) ((Itmp(j), J=1,4), I=1, NOSEG) 
    48         READ (IRD+10) ((SPNAME(I,J), I=1,10), J=1,NOSPEC) 
    49         NXY=NOXG*NOYG 
    50         allocate(A1(NOSPEC,NXY)) 
    51       ird=2 
    52        nam0(3)='EMISSIONS ' 
    53        print*,NAMAV 
    54         do i=1,10 
    55           NAMAV(i)=nam0(3)(i:i)//'   ' 
    56         enddo 
    57        print*,NAMAV 
    58 
    59         WRITE(IRD+10) NAMAV, MRUNID, NOSEG, NOSPEC, 
    60      +    NDATE2(1), 0., 
    61      $    NDATE2(1)+1, 1. 
    62         WRITE(IRD+10) XUTM, YUTM, NZONE, XORG, YORG, DELTAX, DELTAY, 
    63      $    NOXG, NOYG, NOZ, NVLOW, NVUP, DZSURF, DZMINL, DZMINU 
    64         WRITE(IRD+10) ((Itmp(j), J=1,4), I=1, NOSEG) 
    65         WRITE(IRD+10) ((SPNAME(I,J), I=1,10), J=1,NOSPEC) 
    66       do IM=1,12 
    67         READ (11,end=33) NDATE1, TAVE1, NDATE, TAVE 
    68         print*,NDATE1, TAVE1, NDATE, TAVE 
    69           DO  L=1,NOSPEC 
    70             DO  K=1,NOZ 
    71               READ(11)ISEG,(MRUNID(I),I=1,10),(A1(L,I),I=1,NXY) 
    72             enddo 
    73           enddo 
    74         if (IM.eq.mon) then 
    75           do ih=0,24 
    76            h=real(ih) 
    77            write(12)NDATE2(1)+int(ih/24), amod(h,24.), NDATE2(1)+int((ih+1)/24), amod(h+1,24.) 
    78            DO  L=1,NOSPEC 
    79              DO  K=1,NOZ 
    80               write(12)iseg,(SPNAME(i,l),i=1,10),(A1(L,i),i=1,NXY) 
    81              enddo 
    82            enddo 
    83          enddo 
    84         endif 
...
- 輸入檔案argment: 月份(2 digit)uamiv 各類別各月份d1、d2檔案(前述mozart2camx轉檔與合併結果)fortBE.?13樣版
...
- 此版本直接處理uamiv格式檔案。
	- 沿用原uamiv時間標籤
	- 由pncgen轉成nc檔之後，還需要再加上必須的全域屬性，因此放棄不持續發展，改由REAS2hr_TimVar.py為之。

```python
kuang@node03 /nas1/camxruns/2016_v7/emis
$ cat -n REAS2hr.py
     1  import os,sys,datetime
     2  import numpy as np
     3  from PseudoNetCDF.camxfiles.Memmaps import uamiv
     4
     5  mm=sys.argv[1]
     6  for d in ['1', '2']:
     7    for cate in ['area', 'avi', 'ind','line', 'ship']:
     8      fname=cate+'/fortBE.'+d+'13_REAS3.base'+mm
     9      if cate=='ship':
    10        fname=cate+'/fortBE.'+d+'13_STEAM.base'+mm
    11      nc= uamiv(fname, 'r+')
    12      V=[list(filter(lambda x:nc.variables[x].ndim==j, [i for i in nc.variables])) for j in [1,2,3,4]]
    13      nt,nlay,nrow,ncol=nc.variables[V[3][0]].shape
    14
    15      fnameI='/nas1/TEDS/REAS3.1/join_spec/d'+d+'L.'+cate+'_'+mm
    16      if cate=='ship':fnameI='/nas1/TEDS/en.ilmatieteenlaitos.fi/surveying-maritime-emissions/output/2015'+mm+'.emis.grd0'+d
    17      nc1= uamiv(fnameI, 'r')
    18      V1=[list(filter(lambda x:nc1.variables[x].ndim==j, [i for i in nc1.variables])) for j in [1,2,3,4]]
    19      nt1,nlay1,nrow1,ncol1=nc1.variables[V1[3][0]].shape
    20      ss=set(V1[3])-set(V[3])
    21      if len(ss) !=0:
    22        for v in ss:
    23          a=np.sum(nc1.variables[v][0,:,:,:],axis=(0,1,2))
    24          if a!=0:
    25            print('spec not found: '+v )
    26            if v in ['CH4','TOLA', 'XYLA','BNZA','ETHY']:continue
    27            sys.exit('unknown spec')
    28
    29      for v in V[3]:
    30        v1=v
    31        if v in ['TOL','XYL']:v1=v1+'A'
    32        if v == 'BENZ':v1='BNZA'
    33        if v == 'ETH':v1='ETHY'
    34        if v1 not in V1[3]:
    35          nc.variables[v][:]=0
    36          continue
    37        avrg1=np.mean(nc1.variables[v1][0,:,:,:],axis=(0,1,2))
    38        if avrg1==0.:
    39          nc.variables[v][:]=0.
    40          continue
    41
    ...
    44            nc.variables[v][t,:,:,:]=nc1.variables[v1][0,:,:,:]
    45        elif cate=='line':
    46          avrg=np.mean(nc.variables[v][:,:,:,:],axis=(0,1,2,3))
    47          if avrg==0.:
    48            for t in range(nt):
    49              nc.variables[v][t,:,:,:]=nc1.variables[v1][0,:,:,:]
    50          else:
    51            for t in range(nt):
    52              a=np.mean(nc.variables[v][t,:,:,:],axis=(0,1,2))
    53              fac=a/avrg
    54              nc.variables[v][t,:,:,:]=nc1.variables[v1][0,:,:,:]*fac
    55        elif cate=='ship':
    56          sdate1=[nc1.variables['TFLAG'][t,0,0] for t in range(nt1)]
    57          for t in range(nt):
    58            sdate=nc.variables['TFLAG'][t,0,0]
    59            if sdate not in sdate1:sys.exit('sdate not found: '+str(sdate))
    60            t1=sdate1.index(sdate)
    61            nc.variables[v][t,:,:,:]=nc1.variables[v1][t1,:,:,:]
    62      nc.close()
```
- add_ncatt.cs
	- REAS2hr.py完成後所生檔案為uamiv格式，再利用pncgen將uamiv檔案成為nc檔之後，仍須加入以下必要之全域屬性，方能進行空品模擬。
...
test $2=='aver' ch='AVERAGE   CAMx 6.40 CMU CB05 RUN' 
 ...
 ncatted -a ITZON,global,o,i,0 $nc 
...
- 本程式與前述REAS2hr.py功能相同，然以nc檔版模為主體，而非以uamiv模版，
	- 重新編寫時間標籤以符合CAMx及CMAQ模式所需
	- 如此可保有完整屬性訊息，(未來)不必再轉nc檔
- 如要輸出nc檔案，在程式最後直接另存檔名即可，不必再進行pncgen。

```python
kuang@node03 /nas1/camxruns/2016_v7/emis 
$ cat -n REAS2hr_TimVar.py 
     1 import os, sys 
     2 import numpy as np 
     3 from PseudoNetCDF.camxfiles.Memmaps import uamiv 
     4 import netCDF4 
    ...
    21 
    22 pncg=subprocess.check_output('which pncgen',shell=True).decode('utf8').strip('\n') 
    23 
    24 part=['CCRS','CPRM', 'FCRS', 'FPRM', 'NA', 'PCL', 'PEC', 'PNO3', 'POA', 'PSO4', 'SULF'] 
    25 
    26 mm=sys.argv[1] 
    27 mo=int(mm) 
    28 ntm=(monthrange(2016,mo)[1]+2)*24+1 
    29 df=read_csv('d4LTimVar.csv') 
    30 sdt=list(df.sdt) #juldate time 
    31 bdate=datetime.datetime(2016,mo,1)+datetime.timedelta(days=-1+8./24) 
    32 edate=bdate+datetime.timedelta(days=monthrange(2016,mo)[1]+3) 
    33 for d in ['1', '2']: 
    34   fnameT='template_d'+d+'.nc' 
    35   for cate in ['area', 'avi', 'ind','line']: 
    36     os.system('cp '+fnameT+' tmp.nc') 
    37     nc = netCDF4.Dataset('tmp.nc', 'r+') 
    38     V=[list(filter(lambda x:nc.variables[x].ndim==j, [i for i in nc.variables])) for j in [1,2,3,4]] 
    39     nt,nlay,nrow,ncol=nc.variables[V[3][0]].shape 
    40     nv=len(V[3]) 
    41 
    42     nc.SDATE,nc.STIME=dt2jul(bdate) 
    43     nc.EDATE,nc.ETIME=dt2jul(edate) 
    44     if 'ETFLAG' not in V[2]: 
    45       zz=nc.createVariable('ETFLAG',"i4",("TSTEP","VAR","DATE-TIME")) 
    46     for t in range(ntm): 
    47       sdate,stime=dt2jul(bdate+datetime.timedelta(days=t/24.)) 
    48       nc.variables['TFLAG'][t,:,0]=[sdate for i in range(nv)] 
    49       nc.variables['TFLAG'][t,:,1]=[stime for i in range(nv)] 
    50       ndate,ntime=dt2jul(bdate+datetime.timedelta(days=(t+1)/24.)) 
    51       nc.variables['ETFLAG'][t,:,0]=[ndate for i in range(nv)] 
    52       nc.variables['ETFLAG'][t,:,1]=[ntime for i in range(nv)] 
    53 
    54     fnameI='/nas1/TEDS/REAS3.1/join_spec/d'+d+'L.'+cate+'_'+mm 
    55     nc1= uamiv(fnameI, 'r') 
    56     V1=[list(filter(lambda x:nc1.variables[x].ndim==j, [i for i in nc1.variables])) for j in [1,2,3,4]] 
    57     nt1,nlay1,nrow1,ncol1=nc1.variables[V1[3][0]].shape 
    58     ss=set(V1[3])-set(V[3]) 
    59     if len(ss) !=0: 
    60       for v in ss: 
    61         a=np.sum(nc1.variables[v][0,:,:,:],axis=(0,1,2)) 
    62         if a!=0: 
    63           print('spec not found: '+v ) 
    64           if v in ['CH4','TOLA', 'XYLA','BNZA','ETHY']:continue 
    65           sys.exit('unknown spec') 
    66 
    67     for v in V[3]: 
    68       nc.variables[v].units='gmole/hr' 
    69       if v in part:nc.variables[v].units='g/hr' 
    70       v1=v 
    71       if v in ['TOL','XYL']:v1=v1+'A' 
    72       if v == 'BENZ':v1='BNZA' 
    73       if v == 'ETH':v1='ETHY' 
    74       if v1 not in V1[3]: 
    75         nc.variables[v][:]=0 
    76         continue 
    77       avrg1=np.mean(nc1.variables[v1][0,:,:,:],axis=(0,1,2)) 
    78       if avrg1==0.: 
    79         nc.variables[v][:]=0. 
    80         continue 
    81 
    82       if cate!='line': 
    83         for t in range(ntm): 
    84           nc.variables[v][t,:,:,:]=nc1.variables[v1][0,:,:,:] 
    85       elif cate=='line': 
    86         if v1 not in df.columns: 
    87           nc.variables[v][:,]=0. 
    88         else: 
    89           for t in range(ntm): 
    90             jdt=int(nc.variables['TFLAG'][t,0,0]*100+nc.variables['TFLAG'][t,0,1]/10000) 
    91             if jdt <sdt[0]:jdt=int(nc.variables['TFLAG'][t+24,0,0]*100+nc.variables['TFLAG'][t+24,0,1]/10000) 
    92             if jdt >sdt[-1]:jdt=int(nc.variables['TFLAG'][t-48,0,0]*100+nc.variables['TFLAG'][t-48,0,1]/10000) 
    93             idx=sdt.index(jdt) 
    94             if type(df.loc[idx,v1])==np.float64: fac=df.loc[idx,v1] 
    95             if type(df.loc[idx,v1])==list:fac=list(df.loc[idx,v1])[0] 
    96             nc.variables[v][t,:,:,:]=nc1.variables[v1][0,:,:,:]*fac 
    97     nc.NAME='EMISSIONS ' 
    98     nc.NOTE='REASV3 FILE'+(60-11)*' ' 
    99     nc.close() 
   100     nc1.close() 
   101     fname=cate+'/fortBE.'+d+'13_REAS3.base'+mm 
   102     os.system(pncg+' -O --out-format=uamiv tmp.nc '+fname)
```
...
$cat -n txt2sum.py 
     1  """ 
     2  This version txt2nc is writing the nc file with Mozart format. 
     3  """ 
    ...
    12 
    13 
    14  path={'114-32-164-198.HINET-IP.hinet.net':'/opt/anaconda3/bin/', 'node03':'/usr/bin/','master':'/cluster/netcdf/bin/', \ 
    15        'DEVP':'/usr/bin/','centos8':'/opt/anaconda3/envs/py37/bin/'} 
    16  hname=subprocess.check_output('echo $HOSTNAME',shell=True).decode('utf8').strip('\n') 
    17  if hname not in path: 
    18    sys.exit('wrong HOSTNAME') 
    19 
    20 
    21 
    22  date0='0000-01-01' 
    23 
    24 fname='all_file.nam' 
    25 with open(fname) as ftext: 
    26   fnames=[line.strip('\n') for line in ftext] 
    27 fname='REAS2MOZ.csv' 
    28 df=read_csv(fname) 
    29 df.columns=['REAS','MOZ','wt'] 
    30 specREAS_MOZ={} 
    31 wt_MOZ={} 
    32 for i in range(len(df)): 
    33   specREAS_MOZ.update({df.loc[i,'REAS']:df.loc[i,'MOZ']}) 
    34   wt_MOZ.update({df.loc[i,'MOZ']:df.loc[i,'wt']}) 
    35 facG=10**6 /1E6 
    36 unit_REAS={i:facG*mw for i,mw in zip(df.REAS,df.wt)} 
    37 with open('aero_fac.txt','r') as f: 
    38   facP={i.split()[0]:float(i.split()[1])/1E6 for i in f} 
    ...
    45 
    46 fname='cate.csv' 
    47 dfc=read_csv(fname) 
    48 #d1 domain 91.23~150.75 
    49 xmin=int( 91.23/0.25)*0.25 
    50 ymin=int(  0.05/0.25)*0.25 
    51 xmax=int(150.75/0.25+1)*0.25 
    52 ymax=int( 45.77/0.25+1)*0.25 
    53 coord_file = Path("./coord.txt") 
    54 try: 
    55   my_abs_path = coord_file.resolve() 
    56 except: 
    57   print ('generating nx,ny,x0,y0...') 
    58   # doesn't exist 
    59   x0,y0=set([xmin,xmax]),set([ymin,ymax]) 
    60   for fname in fnames: 
    61 #  if stop==1:break 
    62     with open(fname) as text_file: 
    63       d=[line.strip('\n').split() for line in text_file] 
    64     f1=int(d[0][0]) 
    65     x0=set([min(max(float(d[l][0]),xmin),xmax) for l in range(f1,len(d))])|x0 
    66     y0=set([min(max(float(d[l][1]),ymin),ymax) for l in range(f1,len(d))])|y0 
    67   x0=list(x0);x0.sort() 
    68   y0=list(y0);y0.sort() 
    69   nx,ny=int((x0[-1]-x0[0])/0.25)+1,int((y0[-1]-y0[0])/0.25)+1 
    70   x0=[x0[0]+0.25*float(i) for i in range(nx)] 
    71   y0=[y0[0]+0.25*float(i) for i in range(ny)] 
    72   with open('coord.txt','w') as ftext: 
    73     ftext.write( "%s" % str(nx)+' '+str(ny)+'\n') 
    74     for i in range(nx): 
    75       ftext.write( "%s" % str(x0[i])+' ') 
    76     ftext.write( "%s" % '\n') 
    77     for i in range(ny): 
    78       ftext.write( "%s" % str(y0[i])+' ') 
    79 else: 
    80   print ('reading nx,ny,x0,y0...') 
    81   # exists 
    82   with open('coord.txt','r') as ftext: 
    83     d=[line.strip('\n').split() for line in ftext] 
    84   nx,ny=int(d[0][0]),int(d[0][1]) 
    85   x0=[float(d[1][i]) for i in range(nx)] 
    86   y0=[float(d[2][i]) for i in range(ny)] 
    87 
    88 #generate the area(km^2) of each grid cells, 0.25 squre = 500~800Km^2 
    89 Latitude_Pole, Longitude_Pole = 23.61000, 120.9900 
    90 pnyc = Proj(proj='lcc', datum='NAD83', lat_1=10, lat_2=40, lat_0=Latitude_Pole, lon_0=Longitude_Pole, x_0=0, y_0=0.0) 
   ...
   104 
   105 nm=12    
   106 HRS=[24*monthrange(2015,m+1)[1] for m in range(nm)] 
   107 fname='./frame.nc' 
   108 a=Dataset(fname,'r') 
   109 v=[i for i in a.variables] 
   110 dm={u'ilev':2, u'lat':ny, u'lev':1,u'lon':nx,'time':nm} 
   111 dtmap={'float32':'f4','float64':'f8','int32':'i4'} 
   112 
   113 stop=0 
   114 dayb=736695-364 
   115 iy0=2015 
   116 day0=date(iy0,1,1) 
   117 for cate in list(dfc['cate']): 
   118   if len([i for i in fnames if cate in i])==0:continue 
   119   if os.path.isfile(cate+".nc"):continue 
   120   if stop==1:break 
   121   try: 
   122     f=Dataset(cate+".nc","w") 
   123   except: 
   124     f.close() #in case of loop-skipping and the file is not-colsed intime yet 
   125     try: 
   126       f=Dataset(cate+".nc","w") 
   127     except: #in case file is hang by other program 
   128       continue #try next category 
   129 
   130   for i in dm: 
   131     f.createDimension(i,dm[i]) 
   132   for i in v: 
   133     io=i 
   134     if str(i)=='NOX':io='T'#the only V[3] 
   135     f.createVariable(io,dtmap[str(a.variables[i].dtype)],a.variables[i].dimensions) 
   136     ln=a.variables[i].ncattrs() 
   137     if len(ln)>0: 
   138       for j in ln: 
   139         val=a.variables[i].getncattr(j) #save the original attributes 
   140         f.variables[io].setncattr(j,val) 
   141     if a.variables[i].ndim ==0: 
   142       f.variables[i][:]=a.variables[i][:] 
   143     if i=='lev':f.variables[i][:]=a.variables[i][-1] 
   144     if i=='ilev':f.variables[i][:]=a.variables[i][-2:] 
   145     if i=='mdt':f.variables[i][:]=[30*3600] 
   146     if io=='T': 
   147       f.variables[io][:,:,:,:]=np.ones(shape=(nm,1,ny,nx))*298. 
   148       f.variables[io].long_name='temperature' 
   149       f.variables[io].units='K' 
   150     if f.variables[io].size==nm: 
   151       if i=='time': 
   152 #       f.variables[i][:]=[(date(iy0,m+1,1)-day0).days+dayb for m in range(12)] 
   153         f.variables[i][:]=[30*m+dayb for m in range(nm)] 
   154       elif i=='date': 
   155         f.variables[i][:]=[int((date(iy0,1,1)+timedelta(days=30*m)).strftime('%Y%m%d')) for m in range(nm)] 
   156       elif i=='datesec': 
   157         f.variables[i][:]=[int(0) for m in range(nm)] 
   158       elif i=='timestep_index': 
   159         f.variables[i][:]=[int(m) for m in range(nm)] 
   160       else: 
   161         f.variables[i][:]=a.variables[i][:nm] 
   162   f.variables['lon'][:]=x0 
   163   f.variables['lat'][:]=y0 
   164   for fname in fnames: 
   165     if 'POWER_PLANTS_POINT' in fname:continue 
   166     if cate not in fname:continue 
   167 #read the txt file 
...
kuang@114-32-164-198 /Users/TEDS/REAS3.1/ems_tmp
```
$ cat -n mkMon2.py
     ...
     4  import datetime
     5  import subprocess
     6  from PseudoNetCDF.camxfiles.Memmaps import uamiv
    ...
    22
    23  #hourly T2
    24  wrf = netCDF4.Dataset('T2.nc','r')
    25  ntw,nroww,ncolw=(wrf.variables['T2'].shape[i] for i in range(3))
    26  #check the start time of wrf file
    27  s=''
    28  for i in wrf['Times'][0][:]:
    29    s+=i.decode('utf8')
    30  if s!='2015-12-31_00:00:00' :sys.exit('wrf start time not right')
    31  beg=datetime.datetime(2015,12,31)
    32  wrf_time=[beg+datetime.timedelta(days=t/24.) for t in range(ntw)]
    33  T2=np.array(wrf.variables['T2'][:,:,:])
    34  one=np.ones(shape=T2.shape)
    35  T2Hi=one/T2
    36  wrf.close()
    37
    38  #monthly T2
    39  T2Mi=np.zeros(shape=(12,nroww,ncolw))
    40  for m in range(12):
    41    mdate=np.array([t for t in range(ntw) if wrf_time[t].month==m+1])
    42    T2Mi[m,:,:]=np.mean(T2[mdate,:,:],axis=0)
    43
    44  #total REAS monthly emission are store in the first 12 records in fortBE.?13.teds10.base00.nc
    45  #base00 is the result of $bs/data/emis/camx2cmaqd?.job
    46  fname='fortBE.213.teds10.base00.nc'
    47  nc = netCDF4.Dataset(fname,'r')
    48  Vm=[list(filter(lambda x:nc.variables[x].ndim==j, [i for i in nc.variables])) for j in [1,2,3,4]]
    49  nt,nlay,nrow,ncol=(nc.variables[Vm[3][0]].shape[i] for i in range(4))
    50  if nrow<nroww:
    51    ib=int((nroww-nrow)/2)
    52    T2Hi=T2Hi[:,ib:-ib,:]
    53    T2Mi=T2Mi[:,ib:-ib,:]
    54  if ncol<ncolw:
    55    ib=int((ncolw-ncol)/2)
    56    T2Hi=T2Hi[:,:,ib:-ib]
    57    T2Mi=T2Mi[:,:,ib:-ib]
    58  slope=np.zeros(shape=(nc.NVARS,nrow,ncol))
    59  intrc=np.zeros(shape=(nc.NVARS,nrow,ncol))
    60  r_sqr=np.zeros(shape=(nc.NVARS,nrow,ncol))
    61  VMn=np.zeros(shape=(nc.NVARS,12,nrow,ncol))
    62  #store the monthly mean emissions
    63  for v in range(nc.NVARS):
    64    if np.max(nc.variables[Vm[3][v]][:12,0,:,:])==0.: continue
    65    VMn[v,:,:,:]=nc.variables[Vm[3][v]][:12,0,:,:]
    66    for j in range(nrow):
    67      for i in range(ncol):
    68        Y=nc.variables[Vm[3][v]][:12,0,j,i]
    69        X=T2Mi[:,j,i].reshape(-1,1)
    70        model = LinearRegression().fit(X, Y)
    71        r_sqr[v,j,i] = model.score(X, Y)
    72        intrc[v,j,i] = model.intercept_
    73        slope[v,j,i] = model.coef_
    74  nc.close()
    75
    76  #prepare the template
    77  mon=int(sys.argv[1])
    78  MM='{:02d}'.format(mon)
    79  fname='fortBE.213.teds10.base'+MM+'.nc'
    80  nc = netCDF4.Dataset(fname,'r+')
    81  V=[list(filter(lambda x:nc.variables[x].ndim==j, [i for i in nc.variables])) for j in [1,2,3,4]]
    82  nt,nlay,nrow,ncol=(nc.variables[V[3][0]].shape[i] for i in range(4))
    83
    84  begdt=jul2dt([nc.SDATE,nc.STIME])
    85
    86  for v in V[3]:
    87    nc.variables[v][:,0,:,:]=np.zeros(shape=(nt,nrow,ncol))
    88  for v in range(nc.NVARS):
    89    if V[3][v] not in Vm[3]:continue
    90    iv=Vm[3].index(V[3][v])
    91    if np.max(VMn[iv,:,:,:])==0.: continue
    92    for j in range(nrow):
    93      for i in range(ncol):
    94        if r_sqr[iv,j,i]<0.2:
    95          nc.variables[V[3][v]][:,0,j,i]=[VMn[iv,mon-1,j,i] for t in range(nt)]
    96        else:
    97          # calc. the emission with regression slopes and interc.
    98          for t in range(nt):
    99            now=begdt+datetime.timedelta(days=t/24.)
   100            try:
   101              inow=wrf_time.index(now)
   102            except:
   103              inow=0
   104            nc.variables[V[3][v]][t,0,j,i]=max(0.,slope[iv,j,i]*T2Hi[inow,j,i]+intrc[iv,j,i])
   105    print(mon,v,V[3][v],Vm[3][iv])
   106  nc.close()
...
$ cat -n mkMon3.py 
    ...
    17    yr=int(jul/1000) 
    18    ih=int(tm/10000.) 
    19    return datetime.datetime(yr,1,1)+datetime.timedelta(days=int(jul-yr*1000-1))+datetime.timedelta(hours=ih) 
    20 
    21 
    22  #hourly T2 
    23  DOM=sys.argv[2] #DOM=d1 or d2 
    24  wrf = netCDF4.Dataset('/nas1/backup/data/cwb/e-service/btraj_WRFnests/2016T2_'+DOM+'/T2.nc','r') 
    25  ntw,nroww,ncolw=(wrf.variables['T2'].shape[i] for i in range(3)) 
    26  #check the start time of wrf file 
    27  s=''.join([i.decode('utf8') for i in wrf['Times'][0][:]]) 
    28  if s!='2015-12-31_00:00:00' :sys.exit('wrf start time not right') 
    29  beg=datetime.datetime(2015,12,31) 
    30  T2=wrf.variables['T2'][:,:,:] 
    31  one=np.ones(shape=T2.shape) 
    32  T2Hi=one/T2 
    33  wrf_time=[beg+datetime.timedelta(days=t/24.) for t in range(ntw)] 
    34  dtm=np.array([(beg+datetime.timedelta(days=i/24.)).month for i in range(ntw)],dtype=int) 
    35  #monthly T2 
    36  T2=np.zeros(shape=(12,nroww,ncolw)) 
    37  for m in range(1,13): 
    38    idx=np.where(dtm==m) 
    39    T2[m-1,:,:]=np.mean(wrf.variables['T2'][idx[0],:,:]) 
    40  one=np.ones(shape=T2.shape) 
    41  T2Mi=one/T2 
    42  wrf.close() 
    43 
    44  #total REAS monthly emission are store in the first 12 records in fortBE.?13.teds10.base00.nc 
    45  #base00 is the result of $bs/data/emis/camx2cmaqd?.job 
    46  fname='../join_spec/'+DOM+'L.area' 
    47  nc = uamiv(fname,'r') 
    48  Vm=[list(filter(lambda x:nc.variables[x].ndim==j, [i for i in nc.variables])) for j in [1,2,3,4]] 
    49  nt,nlay,nrow,ncol=(nc.variables[Vm[3][0]].shape[i] for i in range(4)) 
    50  if nrow<nroww: 
    ...
    61  VMn=np.zeros(shape=(nc.NVARS,12,nrow,ncol)) 
    62  yr=2016 
    63  mon=int(sys.argv[1]) 
    64  ntm=(monthrange(yr,mon)[1])*24 
    65  #store the monthly mean emissions(gmole/month) 
    66  for v in range(nc.NVARS): 
    67    if np.max(nc.variables[Vm[3][v]][:12,0,:,:])==0.: continue 
    68    VMn[v,:,:,:]=nc.variables[Vm[3][v]][:12,0,:,:] #gmole/hr(monthly avrg) 
    69    for j in range(nrow): 
    70      for i in range(ncol): 
    71        Y=nc.variables[Vm[3][v]][:12,0,j,i] 
    72        X=T2Mi[:,j,i].reshape(-1,1) 
    73        model = LinearRegression().fit(X, Y) 
    74        r_sqr[v,j,i] = model.score(X, Y) 
    75        intrc[v,j,i] = model.intercept_ 
    76        slope[v,j,i] = model.coef_ 
    77  nc.close() 
    78 
    79  #prepare the template 
    80  MM='{:02d}'.format(mon) 
    81  pth='/nas1/camxruns/2016_v7/emis/area/' 
    82  fname=pth+'fortBE.'+DOM[1]+'13_REAS3.base'+MM 
    83  fnameO=fname+'_T2' 
    84  res=os.system('cp '+fname+' '+fnameO) 
    85  nc = uamiv(fnameO,'r+') 
    86  V=[list(filter(lambda x:nc.variables[x].ndim==j, [i for i in nc.variables])) for j in [1,2,3,4]] 
    87  nt,nlay,nrow,ncol=(nc.variables[V[3][0]].shape[i] for i in range(4)) 
    88 
    89  begdt=jul2dt([nc.SDATE,nc.STIME])+datetime.timedelta(days=-8/24.) 
    90 
    91  for v in V[3]: 
    92    nc.variables[v][:,0,:,:]=0. 
    93  now=[begdt+datetime.timedelta(days=t/24.) for t in range(nt)] 
    94  inow=[wrf_time.index(now[t])  for t in range(nt)] 
    95 
    96  for v in range(nc.NVARS): 
    97    if V[3][v] not in Vm[3]:continue 
    98    iv=Vm[3].index(V[3][v]) 
    99    if np.max(VMn[iv,:,:,:])==0.: continue 
   100    z=np.zeros(shape=(nt,nrow,ncol)) 
   101    idx=np.where(r_sqr[iv,:,:]<0.2) 
   102    if len(idx[0])>0: 
   103      z[:,idx[0][:],idx[1][:]]=[VMn[iv,mon-1,idx[0][:],idx[1][:]] for t in range(nt)] 
   104    idx=np.where(r_sqr[iv,:,:]>=0.2) 
   105    if len(idx[0])>0: 
   106    # calc. the emission with regression slopes and interc. 
   107      for t in range(nt): 
   108        a=slope[iv,idx[0][:],idx[1][:]]*T2Hi[inow[t],idx[0][:],idx[1][:]]+intrc[iv,idx[0][:],idx[1][:]] 
   109        z[t,idx[0][:],idx[1][:]]=np.clip(a,0,np.max(a)) 
   110  #promicing the mass conservations, due to non-linear interpolations 
   111    sumv=np.sum(z[:,:,:])/ntm 
   112    sumo=np.sum(VMn[iv,mon-1,:,:]) 
   113    nc.variables[V[3][v]][:,0,:,:]=z[:,:,:]*sumo/sumv 
   114    print(mon,v,V[3][v],Vm[3][iv],sumo,sumv) 
   115  nc.close()
```
   
Reference
